...
def extract_trial_num_within_chord(row):
    ...
    if row['block_type'] == 'purturbed':
        trial_num_within_chord += num_trials_baseline
    return trial_num_within_chord


def determine_two_finger_chord_type(row):
    # check whether num_fingers or num_targets is a column in the dataframe
    if 'num_fingers' in row.index:
        num_fingers = row['num_fingers']
    elif 'num_targets' in row.index:
        num_fingers = row['num_targets']
    else:
        raise ValueError('num_fingers or num_targets is not a column in the dataframe')
...
def finger_melt_Forces(subjs_force: pd.DataFrame) -> pd.DataFrame:
    ...
    subj_force_melted = pd.melt(subjs_force, 
                    id_vars=['state', 'time', 'subNum', 'BN', 'TN', 'trial_num_within_chord', 'chord', 'block_type', 'num_targets',
                             'targetForce1', 'targetForce2', 'targetForce3', 'targetForce4', 'targetForce5',
                             'endForce1', 'endForce2', 'endForce3', 'endForce4', 'endForce5',
                             'TotalTrialNum', 'trialCorr', 'trialErrorType'],
